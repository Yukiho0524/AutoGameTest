...
- **QA**：懶人檢查台。彙整環境、任務、腳本與 TestCase 狀態；生成 TestCase 前需先選擇對應遊戲，再把企劃書（docx / pdf / xlsx / txt / md）丟給 Codex 轉成 QA 用 TestCase xlsx，同步更新該遊戲 Skill。已產出的 TestCase 會記住所屬遊戲，可直接按「執行」建立 `run_agent` 測試任務，讓 Agent 進遊戲導航到該系統並先測前 25 條待測案例。
...
3. testgen：由 `tools/run_testgen.py` 抽取企劃書文字，依 `TESTCASE_SPEC.md` 交給 Codex 設計 CASE，輸出 `TestCase/<企劃書檔名>_TestCase.xlsx`，並把企劃書 QA 重點追加到該遊戲 Skill
4. testcase run：QA 頁執行 TestCase 時，後端會從 xlsx 讀出綁定遊戲與主表案例，取前 25 條未填 PASS/FAIL 的案例組成 `run_agent` 任務；目前結果先寫在任務詳情/log，不直接回寫 Excel
5. 任務檔會標記 `running` / `done` / `error`，並填入 `result`、`engine_used`、`attempts`
