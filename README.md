...
若任務內容是 `1.`、`2.`、`3.` 這類條列步驟，runner 會自動拆成多個 checkpoint 分段執行，降低長任務後段上下文膨脹造成的判斷延遲。每段預設最多等待 180 秒；需要排查時可加 `--no-segment` 回到舊的單輪模式，或用 `--segment-timeout` 調整單段秒數。
...
python tools/run_agent.py --agent <id> --no-segment          # 停用條列任務自動分段
