...
- **QA**：懶人檢查台。彙整環境、任務、腳本與 TestCase 狀態；可把企劃書（docx / pdf / xlsx / txt / md）丟給 Codex 轉成 QA 用 TestCase xlsx，完成後在清單下載。已產出的 TestCase 可選遊戲後按「執行」，系統會建立 `run_agent` 測試任務，讓 Agent 進遊戲導航到該系統並先測前 25 條待測案例。
...
4. testcase run：QA 頁選遊戲執行 TestCase 時，後端會讀 xlsx 主表，取前 25 條未填 PASS/FAIL 的案例組成 `run_agent` 任務；目前結果先寫在任務詳情/log，不直接回寫 Excel
