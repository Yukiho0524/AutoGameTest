...
Agent 也可開啟「自主探索模式」。此模式可讓 prompt 留空，runner 會自動使用快速逐圖架構，讓 AI 每輪看最新截圖、自己選擇低風險入口、記錄看到的 UI/流程/卡點，並在任務結果中回饋 `observation` / `learned`。自主探索不再使用固定輪數上限，而是以設定頁的 AI timeout 或 CLI `--timeout` 作為總探索時間；遇到登入、PVP、未知或不確定畫面時不會立刻停止，而是先記錄、等待、返回或探索其他安全入口；仍不會代輸帳密、點第三方授權、購買付費商品，或開始會影響真人玩家的排位/匹配。
