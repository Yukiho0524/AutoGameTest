...
5. testcase run：QA 頁執行 TestCase 時，後端會從 xlsx 讀出綁定遊戲、系統 Skill 與主表案例，取前 25 條未填 PASS/FAIL 的案例組成 `run_agent` 任務；破壞性 TestCase 只挑 SAFE 案例；目前結果先寫在任務詳情/log，不直接回寫 Excel
