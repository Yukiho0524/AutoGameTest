...
Agent 可在編輯頁開啟「快速逐圖模式」。此模式僅支援 Android 模擬器：runner 每輪先用 ADB 截圖，開一個新的 Codex 最小對話，只提供遊戲 Skill、任務、上一輪一句摘要與最新截圖，要求 Codex 回 JSON 決策；runner 再解析 JSON 並執行 `tap` / `swipe` / `wait` / `launch_app`。它適合固定流程、每一步都能靠目前畫面判斷的任務，可避免長對話越跑越慢；複雜探索、需要長期策略記憶的任務建議維持一般模式。快速逐圖預設不限制輪數，改以設定頁或 CLI `--timeout` 的總時間為上限；只有手動指定 `--visual-max-turns` 時才會限制最多輪數。快速逐圖啟動前會先檢查/啟動模擬器、等待 ADB ready、啟動 App；若截圖失敗，任務結果會列出 emulator、serial、ADB 路徑、devices 與 screencap 失敗階段，方便在其他電腦對照 `config/local.json`。
Agent 也可開啟「自主探索模式」。此模式可讓 prompt 留空，runner 會自動使用快速逐圖架構，讓 AI 每輪看最新截圖、自己選擇低風險入口、記錄看到的 UI/流程/卡點，並在任務結果中回饋 `observation` / `learned` / `player_feedback`。自主探索不再使用固定輪數上限，而是以設定頁的 AI timeout 或 CLI `--timeout` 作為總探索時間；單輪 Codex 判斷逾時只會記錄該輪 timeout 並繼續探索，不會直接讓整筆任務 error。完成後會在 `data/reports/autonomous/` 產出玩家視角 Excel 回饋報告，任務詳情可直接下載。遇到登入、PVP、未知或不確定畫面時不會立刻停止，而是先記錄、等待、返回或探索其他安全入口；仍不會代輸帳密、點第三方授權、購買付費商品，或開始會影響真人玩家的排位/匹配。
...
Agent job 完成後，若設定頁的「Agent 完成後自動把效能建議交給 Codex 調整」開啟，系統會自動建立 `autotune_agent` job。這個 job 會把該次 `performance_analysis`、原任務結果、Skill/Agent 摘要與已保存的 artifact 截圖候選交給 Codex，讓 Codex 在 read-only sandbox 中輸出保守的結構化調整：`AUTOGAMETEST_SKILL_LESSONS` 由 runner 追加到該遊戲 Skill，`AUTOGAMETEST_VISUAL_MEMORY` 由 runner 合併到圖片記憶，安全可操作記憶再晉升 fast rules。它不會操作遊戲、不會登入/付費/抽卡，也不會自動 git commit / push。
...
python tools/run_agent.py --game <game_id> --autonomous --timeout 3600  # 自主探索：以 timeout 控制總時間
python tools/run_agent.py --agent <id> --fast-visual --visual-turn-timeout 120
python tools/run_agent.py --agent <id> --fast-visual --visual-max-turns 20  # 需要硬性限制輪數時才指定
```
