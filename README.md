...
.codex/
  ...
  skills/<遊戲id>-system-<hash>/SKILL.md # 企劃書生成的系統理解 Skill
...
3. testgen：由 `tools/run_testgen.py` 抽取企劃書文字，依 `TESTCASE_SPEC.md` 交給 Codex 設計 CASE 與系統理解，輸出 `TestCase/<企劃書檔名>_TestCase.xlsx`，並在 `.codex/skills/<遊戲id>-system-<hash>/SKILL.md` 建立/更新該系統要做什麼、入口、核心流程、狀態與風險
4. testcase run：QA 頁執行 TestCase 時，後端會從 xlsx 讀出綁定遊戲、系統 Skill 與主表案例，取前 25 條未填 PASS/FAIL 的案例組成 `run_agent` 任務；目前結果先寫在任務詳情/log，不直接回寫 Excel
