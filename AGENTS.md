...
2. 依 `kind` 執行：
   ...
   - **autotune_agent**：讀 `payload.source_job_id` 指向的 Agent job、`performance_analysis`、Skill/Agent 檔與已保存的 artifact 截圖，做保守的效能知識調整；可評估候選截圖是否輸出 `AUTOGAMETEST_VISUAL_MEMORY` 交由 runner 合併，不可操作遊戲、不可登入/付費/抽卡、不可憑空建立 fast rule、不可 git commit/push。
...
圖片相關記憶存於 `data/visual_memory/<game_id>/memory.json` 與 `images/`。可用 `python tools/visual_memory.py add <game_id> <png> --label ... --note ...` 手動收錄測試截圖，也可用 `python tools/visual_memory.py promote <game_id>` 將安全可操作記憶晉升到 fast rules；`run_learn.py` 與 `run_agent.py` 會把這些畫面狀態、signature、區域與風險標籤放進 prompt。Codex 若確認新畫面，可輸出 `AUTOGAMETEST_VISUAL_MEMORY` JSON 讓 runner 自動合併；`run_autotune.py` 也會在 Agent 結束後評估 artifact 截圖是否值得加入圖片記憶。高風險畫面只能標為風險記憶，不可建立安全自動操作。
