...
2. 依 `kind` 執行：
   ...
   - **testgen**：讀 `payload.doc_path` 指向的企劃書與 `payload.game_id`，依 `TESTCASE_SPEC.md` 用 `tools/run_testgen.py` 生成 `TestCase/<企劃書檔名>_TestCase.xlsx`，並在 `data/system_skills/<game_id>/<遊戲id>-system-<hash>/SKILL.md` 建立/更新該企劃書系統的理解 Skill；只做文件測試設計，不操作遊戲、不登入、不消費。
