...
2. 依 `kind` 執行：
   ...
   - **run_agent**：載入 `.Codex/agents/<game_id>-player.md` 與該遊戲 skill，依遊戲的 `control` 模式操作：
     ...
     - 若 `payload.source == "testcase"`，代表 QA 頁從 TestCase 文件發起測試；先讀 `payload.testcase_system_skill_path` 對應的系統理解 Skill（若有），再依 `payload.prompt` 進入指定遊戲系統逐條驗證，回覆 `RESULT: TC編號|PASS/FAIL/N/A|證據或原因`，runner 會用 RESULT 格式回寫 Excel 的 PASS/FAIL 與備註欄。
   - **testgen**：讀 `payload.doc_path` 指向的企劃書與 `payload.game_id`，依 `TESTCASE_SPEC.md` 用 `tools/run_testgen.py` 生成 `TestCase/<企劃書檔名>_TestCase.xlsx`，並在 `data/system_skills/<game_id>/<遊戲id>-system-<hash>/SKILL.md` 建立/更新該企劃書系統的理解 Skill；若 `payload.mode == "destructive"`，改從 `payload.testcase_name` 讀既有標準 TestCase 生成 `<原名>_DestructiveTestCase.xlsx`，案例需分 `SAFE / GUARDED / MANUAL`。只做文件測試設計，不操作遊戲、不登入、不消費。
   - **autotune_agent**：讀 `payload.source_job_id` 指向的 Agent job、`performance_analysis`、Skill/Agent 檔與已保存的 artifact 截圖，做保守的效能知識調整；子 Codex 只輸出 `AUTOGAMETEST_SKILL_LESSONS` / `AUTOGAMETEST_VISUAL_MEMORY` / `AUTOGAMETEST_AUTOTUNE_SUMMARY`，由 `run_autotune.py` 主流程落檔與合併，不依賴子 Codex 寫檔權限。不可操作遊戲、不可登入/付費/抽卡、不可憑空建立 fast rule、不可 git commit/push。
...
控制台「QA」頁籤的「企劃書 → QA TestCase」會先要求選遊戲，再建立 `kind: "testgen"` job，背景呼叫 `tools/run_testgen.py`，並把產出的 xlsx、企劃書備份、遊戲綁定與系統 Skill 綁定資訊放在 `TestCase/`；`TestCase/_input/` 只放抽取暫存與 log，不進 git。系統 Skill 放在 `data/system_skills/` 而非 `.codex/skills/`，避免系統數量變多時污染 Codex 自動 skill metadata。已產出的標準 TestCase 可再生成破壞性測試 xlsx；執行破壞性 TestCase 時，後端只會把 `SAFE` 案例組成 `source: "testcase"` 的 `run_agent` job，`GUARDED` / `MANUAL` 只供人工審查。
...
圖片相關記憶存於 `data/visual_memory/<game_id>/memory.json` 與 `images/`。可用 `python tools/visual_memory.py add <game_id> <png> --label ... --note ...` 手動收錄測試截圖，也可用 `python tools/visual_memory.py promote <game_id>` 將安全可操作記憶晉升到 fast rules；`run_learn.py` 與 `run_agent.py` 會把這些畫面狀態、signature、區域與風險標籤放進 prompt。Codex 若確認新畫面，可輸出 `AUTOGAMETEST_VISUAL_MEMORY` JSON 讓 runner 自動合併；`run_autotune.py` 也會在 Agent 結束後評估 artifact 截圖是否值得加入圖片記憶，並把 `AUTOGAMETEST_SKILL_LESSONS` 追加到該遊戲 Skill。高風險畫面只能標為風險記憶，不可建立安全自動操作。
