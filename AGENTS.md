...
模擬器 agent 預設會先跑 `core/fast_agent.py` 快速判斷層：啟動遊戲、截圖、比對 `data/fast_rules/<game_id>.json` 的安全規則；命中才直接 ADB 操作，未命中交給 Codex。Codex 若學到安全穩定的固定畫面操作，可在最終輸出 `AUTOGAMETEST_FAST_RULES` JSON，runner 會自動合併。登入、付費、轉蛋、PVP 不建立快速規則。
