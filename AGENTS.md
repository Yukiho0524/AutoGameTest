...
2. 依 `kind` 執行：
   - **learn**：抓 `payload.sources` 的網頁（WebFetch），理解遊戲玩法，生成或更新該遊戲 `skill_path`（預設 `.claude/skills/<game_id>/SKILL.md`，含遊戲概述、啟動流程、UI 地圖、例行任務、風險守則、經驗教訓段）。
...
控制台的「學習」按鈕與新增遊戲時的「儲存後自動建立/更新 Skill」會背景呼叫 `tools/run_learn.py`，自動走 Claude Code → Codex fallback。手動處理 `pending` learn job 時，仍需遵守上述格式與風險守則。
...
`tools/ai_runner.py` 提供 Codex→Codex 的額度用盡自動切換（外部包裝器，僅適用腳本化提示，無法救援互動 session）。呼叫 `run_with_fallback(prompt, cwd, timeout, engine, fallback)`，或 CLI `python tools/ai_runner.py "..."`。
