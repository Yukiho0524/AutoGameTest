...
2. 依 `kind` 執行：
   ...
   - **testgen**：讀 `payload.doc_path` 指向的企劃書，依 `TESTCASE_SPEC.md` 用 `tools/run_testgen.py` 生成 `TestCase/<企劃書檔名>_TestCase.xlsx`；只做文件測試設計，不操作遊戲、不登入、不消費。
...
控制台「QA」頁籤的「企劃書 → QA TestCase」會建立 `kind: "testgen"` job，背景呼叫 `tools/run_testgen.py`，並把產出的 xlsx 與企劃書備份放在 `TestCase/`；`TestCase/_input/` 只放抽取暫存與 log，不進 git。已產出的 TestCase 可在 QA 頁選遊戲後建立 `source: "testcase"` 的 `run_agent` job，讓 Agent 進遊戲到該系統測前 25 條未填結果案例。
